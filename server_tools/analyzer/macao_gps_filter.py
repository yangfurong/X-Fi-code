...
#DB_INPUT = "carfi_macau"
#DB_OUTPUT = "carfi_macau_downtown"

#DB_INPUT = "carfi_macau_downtown"
#DB_OUTPUT = "carfi_macau_merged_downtown"
...
parser.add_argument("-d", "--distance", type=float,  default=DEF_DIS, help="maximum distance from the central point")
...
def main():
    ...
    #find the latest experiment from downtown database
    ...
    for i_exp in i_exps:
        ...
        if len(gps_points) >= 1:
            first_p = gps_points[0]
            last_p = gps_points[-1]
            f_dist = geo_distance((first_p.latitude, first_p.longitude), (CENT_LAT, CENT_LONG)).km
            l_dist = geo_distance((last_p.latitude, last_p.longitude), (CENT_LAT, CENT_LONG)).km
            if f_dist <= args.distance and l_dist <= args.distance:
    ...
    #for each experiment, check the distance between the first GPS point and the central point and the distance between the last GPS point and the central point. If both are less than args.distance, add the experiment and related GPS points and APTCPInfo to the downtown database
