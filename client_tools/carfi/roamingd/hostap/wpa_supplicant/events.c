					    ...
					    int only_first_ssid)
{
	...
	wpa_dbg(wpa_s, MSG_DEBUG, "%d: " MACSTR " ssid='%s' "
		"wpa_ie_len=%u rsn_ie_len=%u caps=0x%x level=%d freq=%d 11g=%s 11n=%s 11ac=%s %s%s%s",
		i, MAC2STR(bss->bssid), wpa_ssid_txt(bss->ssid, bss->ssid_len),
		wpa_ie_len, rsn_ie_len, bss->caps, bss->level, bss->freq,
		wpa_bss_get_ie(bss, WLAN_EID_EXT_SUPP_RATES) ? "yes" : "no",
		wpa_bss_get_ie(bss, WLAN_EID_HT_CAP) ? "yes": "no",
		wpa_bss_get_ie(bss, WLAN_EID_VHT_CAP) ? "yes": "no",
		wpa_bss_get_vendor_ie(bss, WPS_IE_VENDOR_TYPE) ? " wps" : "",
		(wpa_bss_get_vendor_ie(bss, P2P_IE_VENDOR_TYPE) ||
		 wpa_bss_get_vendor_ie_beacon(bss, P2P_IE_VENDOR_TYPE)) ?
		" p2p" : "",
...
//#define CARFI_RANDOM_BSS_PICK

#ifdef CARFI_RANDOM_BSS_PICK
    //add randomness to selection when there are multiple BSSes available.
    struct wpa_bss *bss_candidates[128];
    struct wpa_ssid *bss_ssids[128];
    int bss_candidates_size = 0;
#endif

    struct wpa_bss *selected_bss = NULL;
	for (i = 0; i < wpa_s->last_scan_res_used; i++) {
		struct wpa_bss *tmp_bss = wpa_s->last_scan_res[i];
        struct wpa_ssid *tmp_ssid = wpa_scan_res_match(wpa_s, i, tmp_bss, group,
						    only_first_ssid);
        if (!*selected_ssid) {
            selected_bss = tmp_bss;
            *selected_ssid = tmp_ssid;
        }
#ifdef CARFI_RANDOM_BSS_PICK
        if(bss_candidates_size < 128) {
            bss_ssids[bss_candidates_size] = *selected_ssid;
            bss_candidates[bss_candidates_size++] = bss;
        }
        if(bss_candidates_size == 128) {
            wpa_dbg(wpa_s, MSG_DEBUG, "   Too many candidates. Ingoring following BSSes.");
        }
#else
		//return bss;
#endif
    ...
    if (*selected_ssid) {
		...
		const u8 *ie;
		int osen;
        struct wpa_bss *bss = selected_bss;
		...
		wpa_dbg(wpa_s, MSG_DEBUG, "%d: " MACSTR " ssid='%s' "
			"wpa_ie_len=%u rsn_ie_len=%u caps=0x%x level=%d freq=%d 11g=%s 11n=%s 11ac=%s %s%s%s",
			wpa_s->last_scan_res_used, MAC2STR(bss->bssid), wpa_ssid_txt(bss->ssid, bss->ssid_len),
			wpa_ie_len, rsn_ie_len, bss->caps, bss->level, bss->freq,
			wpa_bss_get_ie(bss, WLAN_EID_EXT_SUPP_RATES) ? "yes" : "no",
			wpa_bss_get_ie(bss, WLAN_EID_HT_CAP) ? "yes": "no",
			wpa_bss_get_ie(bss, WLAN_EID_VHT_CAP) ? "yes": "no",
...
#ifdef CARFI_RANDOM_BSS_PICK
    //randomly pick one from bss_candidates[]
	if(bss_candidates_size > 0) {
		int index = rand() % bss_candidates_size;
		struct wpa_bss *bss = bss_candidates[index];
        *selected_ssid = bss_ssids[index];
		...
		wpa_dbg(wpa_s, MSG_DEBUG, "%d: " MACSTR " ssid='%s' "
				"wpa_ie_len=%u rsn_ie_len=%u caps=0x%x level=%d%s%s%s",
				99, MAC2STR(bss->bssid), wpa_ssid_txt(bss->ssid, bss->ssid_len),
				wpa_ie_len, rsn_ie_len, bss->caps, bss->level,
				wpa_bss_get_vendor_ie(bss, WPS_IE_VENDOR_TYPE) ? " wps" : "",
				(wpa_bss_get_vendor_ie(bss, P2P_IE_VENDOR_TYPE) ||
				 wpa_bss_get_vendor_ie_beacon(bss, P2P_IE_VENDOR_TYPE)) ?
		...
		wpa_dbg(wpa_s, MSG_DEBUG, "   final selected BSS " MACSTR
				" ssid='%s'",
				MAC2STR(bss->bssid),
				wpa_ssid_txt(bss->ssid, bss->ssid_len));
		...
		struct wpa_ssid *ssid)
{
	...
	if (wpa_s->reassociate ||
		...
		if (wpa_supplicant_scard_init(wpa_s, ssid)) {
			wpa_supplicant_req_new_scan(wpa_s, 2, 0); /* ### CarFi ### */
			return 0;
		}
		...
		union wpa_event_data *data)
{
	...
	if (wpa_s->key_mgmt == WPA_KEY_MGMT_NONE ||
		...
		wpa_supplicant_req_auth_timeout(wpa_s, 2, 0); /* ### CarFi ### */
	}
