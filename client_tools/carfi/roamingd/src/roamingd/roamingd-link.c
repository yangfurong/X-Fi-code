...
static int link_new(Manager *manager, sd_netlink_message *message, Link **ret) {
        ...
        if (asprintf(&link->state_file, "links/%d", link->ifindex) < 0)
                return -ENOMEM;

        if (asprintf(&link->lease_file, "leases/%d", link->ifindex) < 0)
                return -ENOMEM;

#if 0
        if (asprintf(&link->lldp_file, "lldp/%d", link->ifindex) < 0)
                return -ENOMEM;
#endif
...
static int link_initialized_and_synced(sd_netlink *rtnl, sd_netlink_message *m,
        ...
        if (!link->network) {
                r = network_get_by_name(link->manager, link->ifname, &network); /* ### CarFi ### */
                if (r == -ENOENT) {
